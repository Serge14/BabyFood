...
# Read all necessary data files and dictionaries
...
# Select data for report preparation
data = data[PS2 == "DRY FOOD" & PS3 != "DRINKS" & Ynb > 2013 & ValueC > 0]
...
# Aggregate on fields which will be used for reporting
...
collect = dcast(df1, Segment ~ Ynb + Mnb, value.var = c("Volume", "Value"), fun.aggregate = sum)
names(collect)[1] = "Join"

dftemp = dcast(df1, Segment + Company ~ Ynb + Mnb, value.var = c("Volume", "Value"), fun.aggregate = sum)
...
dftemp = dcast(df1, Segment + Company + Brand ~ Ynb + Mnb, value.var = c("Volume", "Value"), 
               fun.aggregate = sum)
...
dftemp = dcast(df1, Segment + Company + House ~ Ynb + Mnb, value.var = c("Volume", "Value"), 
               fun.aggregate = sum)
...
dftemp = dcast(df1, Segment + Company + Brand + House ~ Ynb + Mnb, value.var = c("Volume", "Value"), 
               fun.aggregate = sum)
...
#Calculate additional aggregated columns
